...
// ========== VISTA VISITA (fichada + retiro + firma + cierre) ==========

function VistaVisita({
  linea, lineaId, supabase, online, onVolver
}: {
  linea: any; lineaId: string; supabase: any; online: boolean; onVolver: () => void
}) {
  ...
  async function cerrarVisita() {
    ...
    if (online) {
      ...
      // Enviar comprobante por email (best-effort)
...
export default function RutaPage() {
  ...
  // Estado de navegación interna (sin cambio de URL)
  ...
  function abrirVisita(linea: any) {
  ...
  function volverALista() {
    ...
    cargarRuta() // Recarga datos actualizados
  }
  ...
  // ---- VISTA VISITA ----
  if (vista === 'visita' && lineaSeleccionada) {
    return (
      <VistaVisita
          ...
          {lineas?.map((linea: any, i: number) => {
            ...
            return (
              <div key={linea.id} className="relative flex gap-3 py-3">
                ...
                {esActiva ? (
                  <button onClick={() => abrirVisita(linea)} className="flex-1 text-left rounded-md p-4 transition-transform active:scale-[.98]"
