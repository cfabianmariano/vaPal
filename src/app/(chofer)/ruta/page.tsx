...
function VistaVisita({ linea, lineaId, supabase, online, onVolver }: { linea: any; lineaId: string; supabase: any; online: boolean; onVolver: () => void }) {
  ...
  async function ficharLlegada() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (geoErr) => { setError(geoErr.code === 1 ? 'Permiso de ubicacion denegado.' : 'No se pudo obtener ubicacion.'); setFichando(false) },
  ...
  async function cerrarVisita() {
    ...
    if (online) {
      ...
      if (linea.vales?.id) nuevoEstadoVale = 'en_curso'
  ...
  return (
    <div>
      ...
      {!online && (<div className="rounded-md p-3 mb-4 text-center text-xs font-semibold uppercase tracking-wider" style={{ background: '#FFF3E0', color: '#E65100', border: '1px solid #FFB74D' }}>Sin senal - los datos se guardaran localmente</div>)}
  ...
  return (
    <div>
      <div className="mb-5">
        ...
        {origen === 'local' && (<p className="text-[10px] font-mono uppercase tracking-wider mt-1" style={{ color: 'var(--amber, #C99031)' }}>Datos locales - se actualizara con senal</p>)}
      </div>
