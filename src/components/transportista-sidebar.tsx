...
const NAV_ITEMS = [
  { href: '/transportista', label: 'Vales asignados', icon: '▤' },
  { href: '/transportista/clientes', label: 'Por cliente', icon: '▥' },
]
...
export default function TransportistaSidebar({ empresaNombre }: { empresaNombre: string }) {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full z-40 flex flex-col py-4 w-56 transition-transform duration-200 md:static md:translate-x-0 md:min-h-screen ${open ? "translate-x-0" : "-translate-x-full"}`}
        ...
        <div className="mb-5 px-4">
          <span className="text-xs font-medium" style={{ color: S.muted }}>{empresaNombre}</span>
        </div>
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setOpen(false)}
                ...
                style={{ background: active ? S.active : 'transparent', color: active ? S.activeText : S.text, borderLeft: active ? `3px solid ${S.activeBorder}` : '3px solid transparent', borderRadius: 0 }}>
                <span style={{ fontSize: '11px', opacity: active ? 1 : 0.6 }}>{item.icon}</span>{item.label}
              </Link>
            )
          })}
        </nav>
