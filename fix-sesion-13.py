...
Ejecutar desde la RAIZ del proyecto:
    python3 fix-sesion-13.py

Reemplaza 2 archivos:
  1. src/app/(chofer)/ruta/page.tsx - fonts + colores + sesion expirada
  2. src/app/api/email-comprobante/route.ts - colores del email

Los otros 2 archivos (layout.tsx, chofer-header.tsx) ya estan correctos.
"""
import os
...
print()
print('=' * 50)
print('  VaPal - Sesion 13: aplicando cambios...')
print('=' * 50)
print()

# ============================================================
# ARCHIVO 1: src/app/(chofer)/ruta/page.tsx
# ============================================================

ruta_content = """'use client'
...
function PadFirma({ onConfirmar, onLimpiar }: { onConfirmar: (blob: Blob) => void; onLimpiar?: () => void }) {
  ...
  const mover = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    const ctx = canvasRef.current!.getContext('2d')!
    const pos = getPos(e)
    ctx.lineTo(pos.x, pos.y); ctx.strokeStyle = '#1b3a4b'; ctx.lineWidth = 2; ctx.lineCap = 'round'; ctx.lineJoin = 'round'; ctx.stroke()
  }
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mt-2">
        ...
        <button onClick={confirmar} disabled={!hayTrazo} className="flex-1 py-2.5 rounded-md text-xs font-bold uppercase tracking-wider text-white transition-opacity" style={{ background: hayTrazo ? 'var(--green)' : 'var(--line)' }}>Confirmar firma</button>
      </div>
    </div>
  )
}
function VistaVisita({ linea, lineaId, supabase, online, onVolver }: { linea: any; lineaId: string; supabase: any; online: boolean; onVolver: () => void }) {
  ...
  async function ficharLlegada() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (geoErr) => { setError(geoErr.code === 1 ? 'Permiso de ubicaci\\u00f3n denegado.' : 'No se pudo obtener ubicaci\\u00f3n.'); setFichando(false) },
  ...
  async function cerrarVisita() {
    ...
    if (online) {
      ...
      setDatosCierre({ remitoId: remitoActivo.id, clienteNombre: linea.clientes.nombre, cantidadTotal: remitoActivo.cantidad_total, cantidadBuenos: remitoActivo.cantidad_buenos, cantidadRecuperar: remitoActivo.cantidad_recuperar, cantidadScrap: remitoActivo.cantidad_scrap, estadiaMinutos: estadiaMin, estado: remitoActivo.estado, firmaNombre: remitoActivo.firma_nombre })
      setVisitaCerrada(true)
    } else {
      let nuevoEstadoVale = null
      if (linea.vales?.id) { nuevoEstadoVale = nuevoEstadoLinea === 'completa' ? 'parcial' : 'en_curso' }
      ...
      <div>
        <div className="rounded-md p-4 mb-4" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
          <div className="flex items-center gap-2 mb-2">
            <span className="w-3 h-3 rounded-full" style={{ background: '#1a7a52' }} />
          ...
          <p className="text-xs" style={{ color: 'var(--muted)' }}>Estad\\u00eda: {datosCierre.estadiaMinutos} min</p>
        </div>
        <div className="flex flex-col gap-3">
          <a
            ...
            className="flex items-center justify-center gap-2 w-full py-3 rounded text-white font-semibold text-sm uppercase tracking-wide"
  ...
  return (
    <div>
      ...
      {!online && (<div className="rounded-md p-3 mb-4 text-center text-xs font-semibold uppercase tracking-wider" style={{ background: 'var(--amber)', color: '#fff', border: '1px solid var(--amber)' }}>Sin se\\u00f1al \\u2014 los datos se guardar\\u00e1n localmente</div>)}
      <div className="rounded-md p-4 mb-4" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
        <div className="text-xs font-mono mb-1" style={{ color: 'var(--muted)' }}>{linea.vales?.numero}</div>
        <h2 className="text-xl font-semibold tracking-tight" style={{ color: 'var(--ink)' }}>{linea.clientes.nombre}</h2>
...
export default function RutaPage() {
  ...
  return (
    <div>
      <div className="mb-5">
        <h2 className="text-2xl font-semibold tracking-tight" style={{ color: 'var(--ink)' }}>Mi ruta</h2>
        <p className="text-sm capitalize" style={{ color: 'var(--muted)' }}>{hoy}</p>
        {origen === 'local' && (<p className="text-[10px] font-mono uppercase tracking-wider mt-1" style={{ color: 'var(--amber)' }}>Datos locales \\u2014 se actualizar\\u00e1 con se\\u00f1al</p>)}
      </div>
      {visitaActiva && (<div className="rounded-md p-3 mb-4 text-center text-xs font-semibold uppercase tracking-wider" style={{ background: 'var(--amber)', color: '#fff', border: '1px solid var(--amber)' }}>Tenes una visita en curso - completala antes de pasar al siguiente cliente.</div>)}
          ...
          {lineas?.map((linea: any, i: number) => {
            ...
            return (
              <div key={linea.id} className="relative flex gap-3 py-3">
                <div className="relative z-10 shrink-0 flex flex-col items-center"><div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold" style={{ background: esActiva ? 'var(--green)' : bloqueada ? 'var(--surface-2)' : 'var(--ink)', color: '#fff', opacity: bloqueada ? 0.5 : 1 }}>{linea.orden_ruta ?? i + 1}</div></div>
                {esActiva ? (
                  <button onClick={() => abrirVisita(linea)} className="flex-1 text-left rounded-md p-4 transition-transform active:scale-[.98]" style={{ background: 'var(--surface)', border: '2px solid var(--green)' }}><div className="flex items-start justify-between gap-3"><div><div className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: 'var(--green)' }}>En curso</div><div className="font-semibold" style={{ color: 'var(--ink)' }}>{linea.clientes.nombre}</div><div className="text-xs mt-0.5" style={{ color: 'var(--muted)' }}>{linea.clientes.direccion}{linea.clientes.localidad ? ' \\u2014 ' + linea.clientes.localidad : ''}</div><div className="text-xs mt-1 font-mono" style={{ color: 'var(--muted)' }}>{(linea.vales as any).numero}</div></div><div className="text-right shrink-0"><div className="text-2xl font-bold" style={{ color: 'var(--green)' }}>{pendiente}</div><div className="text-[10px] uppercase tracking-wider" style={{ color: 'var(--muted)' }}>a retirar</div></div></div><div className="mt-3 py-2.5 rounded text-center text-xs font-bold uppercase tracking-wider text-white" style={{ background: 'var(--green)' }}>Continuar visita</div></button>
                ) : bloqueada ? (
                  <div className="flex-1 rounded-md p-4" style={{ background: 'var(--surface)', border: '1px solid var(--line)', opacity: 0.45 }}><div className="flex items-start justify-between gap-3"><div><div className="font-semibold" style={{ color: 'var(--ink)' }}>{linea.clientes.nombre}</div><div className="text-xs mt-0.5" style={{ color: 'var(--muted)' }}>{linea.clientes.direccion}{linea.clientes.localidad ? ' \\u2014 ' + linea.clientes.localidad : ''}</div></div><div className="text-right shrink-0"><div className="text-2xl font-bold" style={{ color: 'var(--muted)' }}>{pendiente}</div><div className="text-[10px] uppercase tracking-wider" style={{ color: 'var(--muted)' }}>a retirar</div></div></div></div>
                ) : (
                  <button onClick={() => abrirVisita(linea)} className="flex-1 text-left rounded-md p-4 transition-transform active:scale-[.98]" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}><div className="flex items-start justify-between gap-3"><div><div className="font-semibold" style={{ color: 'var(--ink)' }}>{linea.clientes.nombre}</div><div className="text-xs mt-0.5" style={{ color: 'var(--muted)' }}>{linea.clientes.direccion}{linea.clientes.localidad ? ' \\u2014 ' + linea.clientes.localidad : ''}</div><div className="text-xs mt-1 font-mono" style={{ color: 'var(--muted)' }}>{(linea.vales as any).numero}</div></div><div className="text-right shrink-0"><div className="text-2xl font-bold" style={{ color: 'var(--green)' }}>{pendiente}</div><div className="text-[10px] uppercase tracking-wider" style={{ color: 'var(--muted)' }}>a retirar</div></div></div>{linea.estado === 'parcial' && (<div className="mt-2 text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--amber)' }}>Retiro parcial \\u2014 quedan {pendiente}</div>)}</button>
                )}
              </div>
            )
          })}
...
write_file('src/app/(chofer)/ruta/page.tsx', ruta_content)


# ============================================================
# ARCHIVO 2: src/app/api/email-comprobante/route.ts
# ============================================================

email_content = """import { NextRequest, NextResponse } from 'next/server'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const htmlEmail = [
      ...
      '      Le informamos que el d\\u00eda <strong>' + fecha + '</strong> se realiz\\u00f3 un retiro de pallets en su establecimiento.' + alertaNoConf,
      '    </p>',
      ...
      '      <tr style="border-bottom: 1px solid #e0e0e0;"><td style="padding: 10px 0; color: #6a8494;">Estad\\u00eda</td><td style="padding: 10px 0; text-align: right;">' + (estadiaMinutos ?? '\\u2014') + ' min</td></tr>',
      '      ' + firmaRow,
      '    </table>',
      ...
      '      Este comprobante fue generado autom\\u00e1ticamente por VaPal. Si tiene consultas, comun\\u00edquese con su proveedor.',
      '    </p>',
      '  </div>',
      '</div>',
    ].join('\\n')
...
write_file('src/app/api/email-comprobante/route.ts', email_content)
...
print()
print('  ruta/page.tsx:')
print('    - Fraunces -> Inter (titulos, numeros)')
print('    - JetBrains Mono eliminado')
print('    - #5D7040 -> #1a7a52 (verde nuevo)')
print('    - #1E2A38 -> #1b3a4b (ink nuevo)')
print('    - #B3261E -> var(--red)')
print('    - #C99031 -> var(--amber)')
print('    - var(--brand) -> var(--green)')
print('    - Fallbacks viejos eliminados')
print('    - Sesion expirada: useEffect checkAuth')
print()
print('  email-comprobante/route.ts:')
print('    - Colores del email HTML actualizados')
print('    - Auth check: sin cambios (ya estaba)')
print()
print('  layout.tsx: sin cambios (ya tenia try/catch)')
print('  chofer-header.tsx: sin cambios (ya tenia cache)')
print()
print('  Siguiente:')
print('    npm run dev')
print('    git add .')
print('    git commit -m "Sesion 13: rediseno chofer + colores email"')
print('    git push')
print('    rm fix-sesion-13.py')
print()
